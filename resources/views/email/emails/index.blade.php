                            ...
                            @foreach($emails as $email)
                                <tr class="group transition-colors {{ $selected && $selected->id === $email->id ? 'bg-blue-600/10' : 'hover:bg-slate-800/50' }}">
                                    <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-300 sm:px-6">
                                        <a href="{{ route('email.emails.index', ['id' => $email->id] + request()->query()) }}" class="block font-medium text-slate-200 hover:text-blue-400">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 text-right text-sm sm:px-6">
                                        <a href="{{ route('email.emails.index', ['id' => $email->id] + request()->query()) }}" class="text-blue-400 hover:text-blue-300">View</a>
                                        ...
                                        <span class="mx-1.5 text-slate-600">|</span>
                                        <form action="{{ route('email.emails.inbound.destroy', $email) }}" method="POST" class="inline" onsubmit="return confirm('Move to Trash?');">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="relative flex min-h-0 flex-1 flex-col">
                    <div class="shrink-0 space-y-4 border-b border-slate-800 p-4 sm:p-6">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <form action="{{ route('email.emails.inbound.destroy', $selected) }}" method="POST" class="inline" onsubmit="return confirm('Move to Trash?');">
                                @csrf
                                @method('DELETE')
